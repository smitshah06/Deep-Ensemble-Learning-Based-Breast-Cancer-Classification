...
wandb.init(project="minor-project-DN169v2", entity="minor-project")
...
# Function with the model architecture of Transfer Learning with DenseNet169_Model architecture
def DenseNet169_Model():
    ...
    output = tf.keras.layers.BatchNormalization()(output)
    output = tf.keras.layers.Dropout(0.25)(output)
    output = tf.keras.layers.Dense(616, activation="relu")(output)
    output = tf.keras.layers.BatchNormalization()(output)
    output = tf.keras.layers.Dropout(0.25)(output)
...
INIT_LR = 0.005 # Changed from 0.001 to 0.005 to get better results
EPOCHS = 200 # Increased from 40 to 200 to get better results
BATCHSIZE = 96 # Increased from 32 to 96 to get better results
...
modelPath = './saved Models/Pretrained DenseNet169'
...
model_checkpoint = tf.keras.callbacks.ModelCheckpoint('./saved Models/Pretrained DenseNet169/DenseNet169-best-model.h5', monitor='val_categorical_accuracy',
...
tf.keras.models.save_model(model, './saved Models/Pretrained DenseNet169/DenseNet169-model.h5', overwrite=True, include_optimizer=True, save_format=None,
...
plt.title('PretrainedDenseNet169 Train/Test Loss')
plt.ylabel('Loss')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Loss/DenseNet169-loss-Graph.png', bbox_inches = "tight")
...
plt.title('PretrainedDenseNet169 Train/Test Accuracy')
plt.ylabel('Accuracy')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Accuracy/DenseNet169-Accuracy-Graph.png', bbox_inches = "tight")
print("Accuracy graph saved to Train-Test History/Accuracy directory")
# plt.show()

# Evaluate the Best Saved Model
model = tf.keras.models.load_model('./saved Models/Pretrained DenseNet169/DenseNet169-model.h5')
...
def plot_confusion_matrix(cm, target_names, title='Confusion matrix', cmap=None, normalize=True):
    ...
    plt.savefig('./Train-Test History/ConfusionMatrix/DenseNet169-cm.png', bbox_inches = "tight")
