...
wandb.init(project="minor-project-xceptionnet-v1", entity="minor-project")
...
# Function with the model architecture of Transfer Learning with XceptionNet121_Model architecture
...
INIT_LR = 0.5
EPOCHS = 600 # Increas 3 or 4x as ResNet paper had 6L Iter for 13L Images
BATCHSIZE = 256 
optimizer = tf.keras.optimizers.Adam(lr=INIT_LR, decay=INIT_LR / EPOCHS)

wandb.config = {
  "learning_rate": 0.5, # Changed from 0.005 to 0.05 to increase the learning for training (v2)
  "epochs": 600, # Increase from 200, to overcome learning plateau
  "batch_size": 256, # Increase from 96 to increase overall complete dataset iteration as a whole
  "optimizer":'adam'
}
...
modelPath = './saved Models/Pretrained XceptionNet'
...
model_checkpoint = tf.keras.callbacks.ModelCheckpoint('./saved Models/Pretrained XceptionNet/XceptionNet-best-model.h5', monitor='val_categorical_accuracy',
...
tf.keras.models.save_model(model, './saved Models/Pretrained XceptionNet/XceptionNet-model.h5', overwrite=True, include_optimizer=True, save_format=None,
...
plt.title('PretrainedXceptionNet Train/Test Loss')
plt.ylabel('Loss')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Loss/XceptionNet-loss-Graph.png', bbox_inches = "tight")
...
plt.title('PretrainedXceptionNet Train/Test Accuracy')
plt.ylabel('Accuracy')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Accuracy/XceptionNet-Accuracy-Graph.png', bbox_inches = "tight")
print("Accuracy graph saved to Train-Test History/Accuracy directory")
# plt.show()

# Evaluate the Best Saved Model
model = tf.keras.models.load_model('./saved Models/Pretrained XceptionNet/XceptionNet-model.h5')
...
def plot_confusion_matrix(cm, target_names, title='Confusion matrix', cmap=None, normalize=True):
    ...
    plt.savefig('./Train-Test History/ConfusionMatrix/XceptionNet-cm.png', bbox_inches = "tight")
...
plot_confusion_matrix(cm=cm, normalize = False, target_names = classes, title= "Confusion Matrix (Pretrained XceptionNet)")
