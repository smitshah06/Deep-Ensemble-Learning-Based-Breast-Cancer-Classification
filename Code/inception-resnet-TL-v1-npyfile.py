...
wandb.init(project="minor-project-inception-resnet-v1", entity="minor-project")
...
# Function with the model architecture of Transfer Learning with InceptionResNetV2121_Model architecture
def InceptionResNetV2_Model():
    baseModel = tf.keras.applications.inception_resnet_v2.InceptionResNetV2(weights="imagenet", include_top=False, input_tensor=tf.keras.layers.Input(shape=(224, 224, 3)))
    # construct the head of the model that will be placed on top of the the base model
    output = baseModel.output
    # x = Conv2D(64, (3,3), activation='relu', padding='same', name='Conv1_1')(output)
    # x = Conv2D(64, (3,3), activation='relu', padding='same', name='Conv1_2')(x)
    # x = tf.keras.layers.AveragePooling2D((4,4), name='pool1')(x)
    # x = SeparableConv2D(128, (3,3), activation='relu', padding='valid', name='Conv2_1')(x)
    # x = SeparableConv2D(128, (3,3), activation='relu', padding='valid', name='Conv2_2')(x)
    # x = tf.keras.layers.AveragePooling2D((4,4), name='pool2')(x)
    # x = SeparableConv2D(256, (3,3), activation='relu', padding='valid', name='Conv3_1')(x)
    # x = BatchNormalization(name='bn1')(x)
    # x = SeparableConv2D(256, (3,3), activation='relu', padding='valid', name='Conv3_2')(x)
    # x = BatchNormalization(name='bn2')(x)
    # x = SeparableConv2D(256, (3,3), activation='relu', padding='valid', name='Conv3_3')(x)
    # x = tf.keras.layers.AveragePooling2D((4,4), name='pool3')(x)
    # x = SeparableConv2D(512, (3,3), activation='relu', padding='valid', name='Conv4_1')(x)
    # x = BatchNormalization(name='bn3')(x)
    # x = SeparableConv2D(512, (3,3), activation='relu', padding='valid', name='Conv4_2')(x)
    # x = BatchNormalization(name='bn4')(x)
    # x = SeparableConv2D(512, (3,3), activation='relu', padding='valid', name='Conv4_3')(x)
    # x = tf.keras.layers.AveragePooling2D((4,4), name='pool4')(x)
    ...
    for layer in baseModel.layers[10:40]:
...
modelPath = '/home/bce19229/19bce229/Code/saved Models/Pretrained InceptionResNetV2'
...
model_checkpoint = tf.keras.callbacks.ModelCheckpoint('/home/bce19229/19bce229/Code/saved Models/Pretrained InceptionResNetV2/InceptionResNetV2-best-model.h5', monitor='val_categorical_accuracy',
...
tf.keras.models.save_model(model, '/home/bce19229/19bce229/Code/saved Models/Pretrained InceptionResNetV2/InceptionResNetV2-model.h5', overwrite=True, include_optimizer=True, save_format=None,
...
plt.title('PretrainedInceptionResNetV2 Train/Test Loss')
plt.ylabel('Loss')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Loss/InceptionResNetV2-loss-Graph.png', bbox_inches = "tight")
...
plt.title('PretrainedInceptionResNetV2 Train/Test Accuracy')
plt.ylabel('Accuracy')
plt.xlabel('Number of Epochs')
plt.legend(loc="upper left")
plt.savefig('./Train-Test History/Accuracy/InceptionResNetV2-Accuracy-Graph.png', bbox_inches = "tight")
print("Accuracy graph saved to Train-Test History/Accuracy directory")
# plt.show()

# Evaluate the Best Saved Model
model = tf.keras.models.load_model('./saved Models/Pretrained InceptionResNetV2/InceptionResNetV2-model.h5')
...
def plot_confusion_matrix(cm, target_names, title='Confusion matrix', cmap=None, normalize=True):
    ...
    plt.savefig('./Train-Test History/ConfusionMatrix/InceptionResNetV2-cm.png', bbox_inches = "tight")
...
plot_confusion_matrix(cm=cm, normalize = False, target_names = classes, title= "Confusion Matrix (Pretrained InceptionResNetV2)")
